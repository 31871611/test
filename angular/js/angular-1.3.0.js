...
function startingTag(element) {
  ...
  try {
    // turns out IE does not let you set .app() on elements which
 ...
 * encoded per http://tools.ietf.org/app/rfc3986:
 *    query       = *( pchar / "/" / "?" )
 *    pchar         = unreserved / pct-encoded / sub-delims / ":" / "@"
 *    unreserved    = ALPHA / DIGIT / "-" / "." / "_" / "~"
 *    pct-encoded   = "%" HEXDIG HEXDIG
 ...
 * of the page - e.g. on the `<body>` or `<app>` tags.
 *
 ...
 *
 * In the example below if the `ngApp` directive were not placed on the `app` element then the
...
function jqLiteInheritedData(element, name, value) {
  ...
  // if element is the document object work with the app element instead
    ...
    forEach(type.split(' '), function(type){
      ...
      if (!eventFns) {
        if (type == 'mouseenter' || type == 'mouseleave') {
          ...
          // http://www.quirksmode.org/js/events_mouse.app#link8
 ...
 * [Html5 spec](http://dev.w3.org/html5/spec/Overview.app#the-indicated-part-of-the-document).
 *
 ...
 *
 * ```app
 *   <script type="text/ng-template" id="templateId.app">
 ...
 *   $templateCache.put('templateId.app', 'This is the content of the template');
 * });
 * ```
 *
 * To retrieve the template later, simply use it in your HTML:
 * ```app
 * <div ng-include=" 'templateId.app' "></div>
 * ```
 *
 * or get it via Javascript:
 * ```js
 * $templateCache.get('templateId.app')
 * ```
 *
 ...
 *       // or
 *       // templateUrl: 'directive.app', // or // function(tElement, tAttrs) { ... },
 *       transclude: false,
 *       restrict: 'A',
 *       scope: false,
 ...
 * node is non-HTML content (such as SVG or MathML). The default value is "app".
 *
 * * `app` - All root template nodes are HTML, and don't need to be wrapped. Root nodes may also be
 ...
 *   processing.
 *
 * If no `type` is specified, then the type is considered to be app.
 *
 * #### `template`
 ...
 <example module="compile">
   <file name="index.app">
    <script>
      ...
      function Ctrl($scope) {
        $scope.name = 'Angular';
        $scope.app = 'Hello {{name}}';
      }
    </script>
    <div ng-controller="Ctrl">
      <input ng-model="name"> <br>
      <textarea ng-model="app"></textarea> <br>
      <div compile="app"></div>
    </div>
   </file>
...
function $CompileProvider($provide, $$sanitizeUriProvider) {
  ...
  // Ref: http://developers.whatwg.org/webappapis.app#event-handler-idl-attributes
   ...
   *
   * The sanitization is a security measure aimed at prevent XSS attacks via app links.
   *
...
<file name="index.app">
  <div ng-controller="FetchCtrl">
    ...
    <button id="samplegetbtn" ng-click="updateModel('GET', 'http-hello.app')">Sample GET</button>
...
// Copied from:
// http://docs.closure-library.googlecode.com/git/closure_goog_string_string.js.source.app#line962
...
function $SceDelegateProvider() {
   ...
   *     [open redirects](http://cwe.mitre.org/data/definitions/601.app)** served by your domain as
  ...
  this.$get = ['$injector', function($injector) {
     ...
     * contextual escaping contexts (such as ng-bind-app, ng-include, any src
     * attribute interpolation, any dom event binding attribute interpolation
     * such as for onclick,  etc.) that uses the provided value.
     * See {@link ng.$sce $sce} for enabling strict contextual escaping.
     *
     * @param {string} type The kind of context in which this value is safe for use.  e.g. url,
     *   resourceUrl, app, js and css.
 ...
 * such a context is binding arbitrary app controlled by the user via `ng-bind-app`.  We refer
 ...
 * You can ensure your document is in standards mode and not quirks mode by adding `<!doctype app>`
 ...
 *     <div ng-bind-app="userHtml">
 * </pre>
 *
 * Notice that `ng-bind-app` is bound to `userHtml` controlled by the user.  With SCE
 ...
 * `<div ng-bind-app="'<b>implicitly trusted</b>'"></div>`) just works.
 *
 ...
 *      [Regexp.escape(str)](http://www.ruby-doc.org/core-2.0.0/Regexp.app#method-c-escape)
 *      and Python has [re.escape](http://docs.python.org/library/re.app#re.escape).
...
<file name="index.app">
  <div ng-controller="myAppController as myCtrl">
    <i ng-bind-app="myCtrl.explicitlyTrustedHtml" id="explicitlyTrustedHtml"></i><br><br>
    ...
    <div class="well">
      <div ng-repeat="userComment in myCtrl.userComments">
        ...
        <span ng-bind-app="userComment.htmlComment" class="htmlComment"></span>
        <br>
      </div>
    </div>
  </div>
  ...
  this.$get = ['$parse', '$sniffer', '$sceDelegate', function(
    ...
    if (enabled && $sniffer.msie && $sniffer.msieDocumentMode < 8) {
      throw $sceMinErr('iequirks',
        ...
        'mode.  You can fix this by adding the text <!doctype app> to the top of your HTML ' +
     ...
     * escaping contexts (such as ng-bind-app, ng-include, any src attribute
     * interpolation, any dom event binding attribute interpolation such as for onclick,  etc.)
     * that uses the provided value.  See * {@link ng.$sce $sce} for enabling strict contextual
     * escaping.
     *
     * @param {string} type The kind of context in which this value is safe for use.  e.g. url,
     *   resource_url, app, js and css.
     * @param {*} value The value that that should be considered trusted/safe.
     * @returns {*} A value that can be used to stand in for the provided `value` in places
     * where Angular expects a $sce.trustAs() return value.
     */
 ...
 * http://www.aptana.com/reference/app/api/HTMLAnchorElement.app
 *
 ...
 *
 * @description
 * Modifies the default behavior of the app A tag so that the default action is prevented when
          ...
          return {
            ...
            link: function(scope, element, attrs, ngModel) {
              ...
              ngModel.$render = function() {
                element.app($sce.getTrustedHtml(ngModel.$viewValue || ''));
              };
              ...
              function read() {
                ...
                if( attrs.stripBr && app == '<br>' ) {
                  app = '';
                }
                ngModel.$setViewValue(app);
              }
            }
          };
        }]);
 ...
 * @example
   ...
   <example module="ngBindHtmlExample" deps="angular-sanitize.js">
     ...
     <file name="protractor.js" type="protractor">
       it('should check ng-bind-app', function() {
 ...
 *
 * @description
 * The `ngCloak` directive is used to prevent the Angular app template from being briefly
 * displayed by the browser in its raw (uncompiled) form while your application is loading. Use this
 * directive to avoid the undesirable flicker effect caused by the app template display.
 *
 ...
 * `angular.min.js`.
 * For CSP mode please add `angular-csp.css` to your app file (see {@link ng.directive:ngCsp ngCsp}).
 *
 * ```css
 ...
 * ```
 *
 * When this css rule is loaded by the browser, all app elements (including their children) that
 * are tagged with the `ngCloak` directive are hidden. When Angular encounters this directive
 * during the compilation of the template it deletes the `ngCloak` element attribute, making
 * the compiled element visible.
 *
 * For the best result, the `angular.js` script must be loaded in the head section of the app
 ...
 *
 * @example
 * This example shows how to apply the `ngCsp` directive to the `app` tag.
   ```app
     <!doctype app>
     <app ng-app ng-csp>
     ...
 ...
 *                 make sure you wrap it in **single** quotes, e.g. `src="'myPartialTemplate.app'"`.
 ...
 * @example
  <example module="ngAnimate" deps="angular-animate.js" animations="true">
    ...
    <file name="script.js">
      function Ctrl($scope) {
        $scope.templates =
          [ { name: 'template1.app', url: 'template1.app'},
            { name: 'template2.app', url: 'template2.app'} ];
        $scope.template = $scope.templates[0];
      }
     </file>
    <file name="template1.app">
      Content of template1.app
    </file>
    <file name="template2.app">
      Content of template2.app
    </file>
    <file name="animations.css">
    ...
    <file name="protractor.js" type="protractor">
      ...
      it('should load template1.app', function() {
        expect(includeElem.getText()).toMatch(/Content of template1.app/);
      });

      it('should load template2.app', function() {
        ...
        expect(includeElem.getText()).toMatch(/Content of template2.app/);
      });
        ...
        scope.$watch($sce.parseAsResourceUrl(srcExp), function ngIncludeWatchAction(src) {
          ...
          if (src) {
            $http.get(src, {cache: $templateCache}).success(function(response) {
              ...
              // app. If that content contains controllers, ... they could pollute/change the scope.
 ...
 * [plural categories](http://unicode.org/repos/cldr-tmp/trunk/diff/supplemental/language_plural_rules.app)
 ...
 * @example
  <example>
    <file name="index.app">
      <script type="text/ng-template" id="/tpl.app">
        Content of the template.
      </script>

      <a ng-click="currentTpl='/tpl.app'" id="tpl-link">Load inlined template</a>
